...
func runQuickstartCommand(args []string, global globalOptions, streams Streams) int {
	...
	fs.StringVar(&opts.InviteLabel, "invite-label", "", "label for the first team member invite (prompted if omitted)")
	fs.StringVar(&opts.InviteLabel, "label", "", "alias for --invite-label")
	fs.BoolVar(&opts.RequestedManagement, "management", false, "grant management to the first invite")
	fs.Var(&opts.RequestedScopes, "scope", "scope permission for the first invite as scope=read|write; may be repeated")
...
func printQuickstartHelp(w io.Writer) {
	...
	fmt.Fprintln(w, "  --invite-label VALUE     label for the first team member invite (prompted if omitted)")
	fmt.Fprintln(w, "  --label VALUE            alias for --invite-label")
	fmt.Fprintln(w, "  --management             grant management to the first invite")
	fmt.Fprintln(w, "  --scope VALUE            scope permission for the first invite as scope=read|write; may be repeated")
